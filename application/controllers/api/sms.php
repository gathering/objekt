 ...
 * @Last Modified time: 2015-03-31 15:31:04
 */
	...
	function post_inbound(){
		...
		if($person){
			...
			if($responsible = $profile->responsible()){
				Notification::send($responsible->id, 'SMS mottatt fra '.$person->firstname.' '.substr($person->surname, 0, 1).'.', $sms->message, 'http://app.obj.no/'.$person->url('sms/inbox'));
			}
		}
